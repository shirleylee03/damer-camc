...
//    gtree *after_node = pre_node->parent->next;
    gtree *pre_node = NULL, *after_node = NULL;
    if (add_pos>0){
        pre_node = get_statementNodeFromPos(originFirstStatementNode,add_pos - 1);//函数输入的pos以0开始
        after_node = pre_node->parent->next;
    }else {
        after_node = get_statementNodeFromPos(originFirstStatementNode, 0);
    }
    /// comment by hz, 2022/07/15
...
string change_del_single(CPN *cpn, AST_change &change, pair<int,int> del){
    ...
    if(del_pos == 1){
        ...
        if(pre_node->type != STATEMENT) {
            if (pre_node->child->type != STATEMENT)
                pre_node = pre_node->child->next;
            else
                ///   del_node is the second statement of this compound and
                /// it's pre_node should be the only child of del_node's brother
                ///
                /// comment by hz 2022/07/13
